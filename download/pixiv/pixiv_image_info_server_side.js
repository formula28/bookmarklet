...
[改版履歴]
0.1 初版.情報は別ページに開く仕様.
0.2 情報をpixivのイラスト個別ページにレイヤー表示するよう変更.
0.3 一部のメタデータをhtml文書からではなく、pixiv.contextから取得するよう修正.
	イラストID,ユーザーIDを表示するよう変更.
	投稿日時を画像ファイルパスから取得し、秒単位まで表示するよう変更.
	レイヤー表示解除のボタンを削除し、代わりにレイヤー上の何もオブジェクトのないところをクリックしたら解除するよう修正.
	画像の個別/まとめて(zip)ダウンロードに対応.
	※個別画像の直接ダウンロード、または、まとめてダウンロードを行うには、CORSを回避するため、どこかで通信をペンディングしてhttpレスポンスヘッダに
	※「Access-Control-Allow-Origin」を追加する必要がある.
0.4 サムネイル画像横の半透過レイヤーをクリックしてもレイヤー表示解除できない不具合修正.
	タグに/が含まれていると、まとめてダウンロードしたZipを解凍した時に/が入るごとにフォルダができてしまう不具合修正.
	うごイラZip保存機能実装.
	※ダウンロードボタンにマウスオーバー/アウトすると周りの表示位置がずれる.原因不明.
	※保存時にファイル名が変更できていない.おそらく非同期通信でダウンロードせずに、一度別タブを開いているせい.
0.5 画像取得先URL解析のもとになるURLを<meta>タグに頼っていたが、pixivページに記載されなくなったため、
	コンテンツ本体の<img>タグから取得する様に修正.
0.6 dropbox直リンク禁止に伴う変更.
*/
//(function(){
	/* ルートURI. */
